...
ccc accuracy specified
        ...
        call complex_root_square(test_fun1,norder,epsexp,
     1          roots,errors,nrtot,centers,nc,c0,sqw,w,
     2          ifdval,ifres,ifnewton)
...
ccc        call prin2('all roots found *',roots,2*nrtot)
        call prin2_long('all roots found *',roots,2*nrtot)
        call prin2('root error is *',errors,nrtot)
...
c        of the complex rootfinding routines
c 
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        subroutine complex_root_square(fun,norder,epsexp,roots,
     1      errors,nrtot,centers,nc,c0,sqw,w,
     2      ifdval,ifres,ifnewton)
        implicit real *8 (a-h,o-z)
        complex *16 c0,roots(1),centers(1)
        dimension w(1)
c       This subroutine is a nonadaptive version of
c       complex_root_adap_square
c
...
c       statisfies some three-term recursions. When the expansion
c       converges to the predetermined accuracy (epsexp), 
c       the root finder finds all roots within the square by finding 
c       eigenvalues of the corresponding colleague matrix. 
...
ccc        call prinf('length is *',lr+ir,1)
        ...
        call complex_root_square0(fun,norder,epsexp,roots,errors,
...
ccc        call get_fekete_pts(zfg,mm)
        ...
        call complex_root_one_square_nonadap(epsexp,ifdone,fun,c0,sqw,
     1      vd,voffd,q,z,cw,n,work,croots,norder,roots,nr,ierqr)

ccc        call prinf('After one_square, ifdone=*',ifdone,1)

cccc    .   .   . Expansion is below the required accuracy
        if(ifdone.eq.0) goto 4100
...
c
 ...
 4100 continue
        ...
        subroutine complex_root_one_square_nonadap(epsexp,ifcv,fun,c0,
     1     sqw,vvd,vvoffd,q,z,cw,n,work,croots,norder,roots,nr,ierqr)
        implicit real *8 (a-h,o-z)
...
ccc        call prin2('coefscut is *',coefscut,1)
        ...
        if(coefscut.gt.eeps) then
          ifcv=0
         call prinf('norder not enough with norder *',norder,1)
         goto 5000
        endif
